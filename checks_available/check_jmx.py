...
jmx_url = config.get('JMX', 'jmx')

cluster_name = config.get('SelfConfig', 'cluster_name')
java = config.get('JMX', 'java')
juser = config.get('JMX', 'user')
jclass = config.get('JMX', 'class')
check_type = 'JMX'
reaction = -3
...
def runcheck():
    try:
        data_dict = json.loads(urllib2.urlopen(jmx_url + '/java.lang:type=GarbageCollector,name=*', timeout=5).read())
        ...
        jolo_url = urllib2.urlopen(jmx_url + '/' + heam_mem, timeout=5).read()
        ...
        if CMS is True:
            ...
            for coltype in collector:
                beans = json.loads(urllib2.urlopen(jmx_url + '/' + coltype, timeout=5).read())
                ...
                def push_metrics(preffix):
                    jsondata.gen_data('jmx_' + preffix + '_CollectionCount', timestamp, CollectionCount, lib.pushdata.hostname, check_type, cluster_name)
                    CollectionTime_rate = rate.record_value_rate('jmx_' + preffix + '_CollectionTime', CollectionTime, timestamp)
                    jsondata.gen_data('jmx_' + preffix + '_CollectionTime', timestamp, CollectionTime_rate, lib.pushdata.hostname, check_type, cluster_name, 0, 'Rate')
        ...
        if G1 is True:
            ...
            for k, v in enumerate(gc_g1):
                j = json.load(urllib2.urlopen(jmx_url + v, timeout=5))
                name = 'LastGcInfo'
                if k is 0:
                    try:
        ...
        jolo_turl = urllib2.urlopen(jmx_url + '/' + jolo_threads, timeout=5).read()
